...
When persona-triggered recommendations appear, show the user a compact summary such as "quant triggered leakage/temporal proof" or "ux_ui triggered browser screenshot proof." Do not bury persona recommendations in JSON artifacts only.
...
| Operational/admin chore (ad budget changes, credential rotations, schedule edits, content tweaks, settings toggles) | **Skip the planner.** Just do the task and commit. If you must use the planner, bootstrap will detect `chore` shape and minimise gates — but this is overkill for actual chores. |
| Question or analysis task (no code change) | **Skip the planner — answer the user or do the read.** Run `bootstrap.mjs triage "<goal>"` first if unsure; v7.4.4+ classifies questions and analysis tasks as skip-the-planner candidates. |
...
When uncertain, run `node .agent/skills/iterative-planner/scripts/bootstrap.mjs triage "<goal>"` to get a complexity score + recommended path BEFORE opening a plan. The triage subcommand is read-only and never writes a plan dir.
...
4. **Reproduce the `deepseek_advisory_block` field verbatim in your reply to the user.** The block is fenced by `<<<DEEPSEEK_VERDICT_BEGIN>>>` and `<<<DEEPSEEK_VERDICT_END>>>` delimiters. Quote them as written — including the delimiters. Paraphrasing or summarising the block is a violation of the receipt's `verbatim_reproduction_contract`. If DeepSeek was unavailable, the block still appears with `Status: unavailable` — reproduce it anyway so the operator knows the LLM review did not run.
5. Deterministic Program Packet, story, annotation, ontology, and verification evidence stays authoritative. DeepSeek or other LLM review is advisory only — but advisory output that survives to the user is only useful if the agent does not silently drop it. Hence the verbatim contract.
