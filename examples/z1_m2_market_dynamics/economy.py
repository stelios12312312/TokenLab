...
class TokenEconomy_Z1(TokenEconomy_Basic):
    ...
    def __init__(self, config: SolvencyConfig):
        ...
        self.campaigns = CampaignEngine(treasury_fee_percentage=config.campaign_fee_percentage)
    ...
    def execute(self) -> bool:
        ...
        fee_to_treasury = self.campaigns.deposit_campaign_funds(self.config.campaign_deposit_per_epoch)
        ...
        for name, pool in cohort_pools.items():
            ...
            requested_acr = pool.acr_available * min(1.0, pool.settle_propensity * panic_multiplier)
        ...
        current_live_supply = self.audience_reserve + self.treasury + current_total_cohort_z1u + self.cumulative_provider_payments + self.amm.z1u_reserve + self.campaigns.escrow_balance_z1u
        
        # Constitutional floor: AR must stay >= 0.25 * live_supply
        # Use a small safety buffer (0.2505) to account for mid-epoch supply changes and float noise.
        ar_min = 0.2505 * current_live_supply
        ...
        ar_ratio_fairness = max_settleable_z1u / total_z1u_after_cap if total_z1u_after_cap > 0 else 1.0
