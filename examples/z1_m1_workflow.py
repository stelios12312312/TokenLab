...
SOLVENCY_DIR = os.path.join(SCRIPT_DIR, "z1_core_solvency")
KB_DIR = os.path.join(SOLVENCY_DIR, "z1_simulation_kb")
OUTPUTS_DIR = os.path.join(SCRIPT_DIR, "outputs", "z1_core_solvency")
...
def run_simulation(scenario="full"):
    ...
    result = subprocess.run(
        [sys.executable, "-m", "examples.z1_core_solvency.run", "--scenario", scenario],
        cwd=os.path.dirname(SCRIPT_DIR),  # TokenLab root
        capture_output=False,
    )
    if result.returncode != 0:
