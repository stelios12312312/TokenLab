...
%%   migration_smoke_satisfied(true/false)
%%   test_evidence_satisfied(true/false)
...
%% EXPLORE → PLAN: requires sufficient findings with depth, KB read proof.
%% v7.3.0: shape-conditional minimum (findings_minimum/1) replaces hardcoded 3.
...
%% PLAN → EXECUTE: requires plan completeness + user approval
...
missing_guard(plan, execute, no_verification_strategy) :-
    \+ verification_strategy(true).
missing_guard(plan, execute, not_approved) :-
    \+ user_approved(true).
...
missing_guard(reflect, validate, semantic_substrate_incomplete) :-
    semantic_substrate_required(true),
    \+ semantic_substrate_satisfied(true).
...
transition_fact(user_approved, V) :- user_approved(V).
...
%% Required predecessor gates — each gate requires the one before it.
