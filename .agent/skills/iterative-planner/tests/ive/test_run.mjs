...
console.log("Scenario A: live tree PASS");
{
  ...
  assert(r.stdout.trim().startsWith("{") && r.stdout.trim().endsWith("}"), "async success path emits one complete JSON document");
...
console.log("\nScenario B: synthetic FAIL via IVE_RUNNER_INJECT_FAILURE");
{
  ...
  assert(r.stdout.trim().startsWith("{") && r.stdout.trim().endsWith("}"), "async failure path emits one complete JSON document before the nonzero exit");
...
console.log("\nScenario C: JSON shape contract");
{
  ...
  assert(!["wave", "wave_results", "execution_mode"].some((key) => key in (r.parsed || {})), "direct scheduling adds no report schema fields");
