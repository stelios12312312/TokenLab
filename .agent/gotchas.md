...
- **Enforcement**: Rule 8 in `.agent/rules.md` (Self-Referential Gate). Pre-commit hook runs `ripple_check.mjs` when planner files are staged. The governed `migration-bootstrap` and `transition-gate-flows` IVE suites validate the migration and planner-transition journeys.
