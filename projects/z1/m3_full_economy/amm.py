class AutomatedMarketMaker:
    ...
    def compute_settlement_ratio(self, base_ratio: float, ar_health: float, config) -> float:
        """
        Compute dynamic settlement ratio based on AMM health and AR health.
        ...
        amm_component = min(1.0, price_health)
        ar_component = min(1.0, ar_health)
        
        composite_health = (amm_component * config.composite_sr_amm_weight) + (ar_component * config.composite_sr_ar_weight)
        
        return base_ratio * composite_health
