"""Shared configuration validation and lock behavior."""

import math
from typing import ClassVar

class SharedConfigBehavior:
    _z1_milestone: ClassVar[str]

    def _cohort_names(self):
        names = ["passive_viewers", "active_viewers", "power_users"]
        if self._z1_milestone != "m1":
            names.append("adversarial_whales")
        return names
    ...
    def validate(self):
        ...
        # Check cohort keys
        assert set(self.cohort_population_shares.keys()) == set(self._cohort_names())
        ...
        mean_settle = sum(self.cohort_population_shares[c] * self.settle_propensity_by_cohort[c] for c in self._cohort_names())
        ...
        mean_spend = sum(self.cohort_population_shares[c] * self.utility_spend_rate_by_cohort[c] for c in self._cohort_names())
        if self._z1_milestone == "m1":
            assert mean_settle <= mean_spend, f"L10 Violation: Weighted settle ({mean_settle:.3f}) > weighted spend ({mean_spend:.3f})"
        elif mean_settle > mean_spend:
            pass  # M2/M3 preserve transient-violation compatibility.
    ...
    def compute_solvency_ratio(self) -> float:
        ...
        """
        outflow = sum(self.cohort_population_shares[c] * self.claim_rate_by_cohort.get(c, 0) * self.settle_propensity_by_cohort.get(c, 0) for c in self._cohort_names()) * self.settlement_ratio

        inflow = sum(self.cohort_population_shares[c] * self.utility_spend_rate_by_cohort.get(c, 0) for c in self._cohort_names()) * self.utility_fee_share + (self.brand_inflow_per_epoch / self.audience_reserve_initial if self.audience_reserve_initial > 0 else 0)
    ...
    def check_solvency_locks(self) -> list[dict]:
        ...
        if self._z1_milestone == "m3" and ratio >= 0.8:
            diagnostics.append({
