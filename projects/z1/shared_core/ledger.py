...
from .policies import LedgerPolicy

GlobalState = Any

def issue_acr_to_vesting(state: GlobalState, cohort_name: str, amount: float, *, _policy: LedgerPolicy):
    if amount <= 0: return
    cohort = state.cohorts[cohort_name]

    phases = (
        getattr(state.config, 'vesting_sub_cohort_phases', 1)
        if _policy.phased_vesting and hasattr(state, 'config')
        else 1
    )
...
def queue_settlement_request(state: GlobalState, cohort_name: str, acr_amount: float, z1u_requested: float, *, _policy: LedgerPolicy):
    ...
    if acr_amount > cohort.acr_available:
        ...
        if _policy.clamp_requested_z1u:
    ...
    if _policy.track_requested_z1u:
...
def execute_settlement(state: GlobalState, cohort_name: str, acr_amount: float, z1u_amount: float, *, _policy: LedgerPolicy):
    ...
    if _policy.normalize_queue_residue:
...
def spend_z1u(state: GlobalState, cohort_name: str, spend_amount: float, provider_payment: float, treasury_fee: float, burn_amount: float, *, _policy: LedgerPolicy):
    ...
    recirculation_rate = (
        getattr(state.config, 'provider_recirculation_rate', 0.0)
        if _policy.provider_recirculation
        else 0.0
    )
    ...
    provider_amm_sell_enabled = _policy.provider_recirculation and getattr(state.config, 'provider_amm_sell_enabled', True)
